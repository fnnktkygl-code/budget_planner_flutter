...
class SalaryRecord {
  ...
  /// Remboursements de frais exonérés (positif, ex: Mobilité / Carburant)
  final double expenseReimbursement;
  /// Avantage employeur tickets resto (positif, pouvoir d'achat)
  final double mealTicketsEmployer;
  /// Rachat de jours de RTT/repos (positif)
  ...
  /// Nom de l'employeur
  String get employerName => "Entreprise Exemple";
  /// Nom du salarié
  String get employeeName => "John Doe";

  /// Taux de conversion empiriques
  static const double conversionRateSalary = 0.759;
  static const double conversionRateBonus = 0.790;
  static const double conversionRateRtt = 0.799;

  /// Flux bancaire réellement perçu (ce qui arrive sur le compte)
  double get actualBankFlow => netSalary;

  /// Pouvoir d'achat réel (Flux bancaire + avantages en nature comme les tickets resto employeur)
  double get purchasingPower => netSalary + mealTicketsEmployer;

  /// Montant d'extra / rachat de congés / prime calculé dynamiquement
  double get calculatedExtraAmount {
    double totalExtra = 0.0;
    if (bonusAmount != null && bonusAmount! > 0) totalExtra += bonusAmount!;
    if (rttBuybackAmount > 0) totalExtra += rttBuybackAmount;
  ...
  bool get isExtraOrBonusMonth =>
      hasExplicitBonus || calculatedExtraAmount > 10.0;

  /// Salaire net récurrent hors prime exceptionnelle (utilise les taux de conversion empiriques)
  double get regularNetSalary {
    if (calculatedExtraAmount > 10.0) {
      ...
      if (bonusAmount != null && bonusAmount! > 0) {
        netDeduction = bonusAmount! * conversionRateBonus;
      } else if (rttBuybackAmount > 0) {
        netDeduction = rttBuybackAmount * conversionRateRtt;
      }
  ...
  /// Net social récurrent hors prime exceptionnelle
  ...
  SalaryRecord copyWith({
    ...
    double? mealTickets,
    double? mealTicketsEmployer,
    double? rttBuybackAmount,
    ...
    return SalaryRecord(
      ...
      mealTicketsEmployer: mealTicketsEmployer ?? this.mealTicketsEmployer,
      rttBuybackAmount: rttBuybackAmount ?? this.rttBuybackAmount,
      ...
      expenseReimbursement: expenseReimbursement ?? this.expenseReimbursement,
  ...
  Map<String, dynamic> toJson({bool includeBinary = true}) => {
        ...
        'mealTicketsEmployer': mealTicketsEmployer,
        'rttBuybackAmount': rttBuybackAmount,
  ...
  factory SalaryRecord.fromJson(Map<String, dynamic> json) => SalaryRecord(
        ...
        mealTicketsEmployer: json['mealTicketsEmployer'] != null ? (json['mealTicketsEmployer'] as num).toDouble() : 0.0,
        rttBuybackAmount: json['rttBuybackAmount'] != null ? (json['rttBuybackAmount'] as num).toDouble() : 0.0,
        ...
        expenseReimbursement: json['expenseReimbursement'] != null ? (json['expenseReimbursement'] as num).toDouble() : 0.0,
