...
class _SalaryAuditScreenState extends ConsumerState<SalaryAuditScreen> {
  ...
  Widget _buildCanvasContent(bool hasFileLoaded) {
    ...
    // 3. CENTERED INTERACTIVE CANVAS VIEWPORT (WITH NATIVE PINCH-TO-ZOOM & PANNING TO ALL EDGES)
    return InteractiveViewer(
      transformationController: _transformationController,
      minScale: 0.8,
      maxScale: 3.5,
      boundaryMargin: const EdgeInsets.all(500),
      panEnabled: _selectedTool == RedactionTool.move || _canvasTab == 1,
      scaleEnabled: true,
  ...
  void _showEditSalaryRecordDialog(BuildContext context, SalaryRecord record) {
    ...
    // Nouveaux champs d'analyse avancée
    final mealEmployerCtrl = TextEditingController(text: record.mealTicketsEmployer > 0 ? record.mealTicketsEmployer.toStringAsFixed(2) : '');
    final expenseReimbursementCtrl = TextEditingController(text: record.expenseReimbursement > 0 ? record.expenseReimbursement.toStringAsFixed(2) : '');
    final rttBuybackCtrl = TextEditingController(text: record.rttBuybackAmount > 0 ? record.rttBuybackAmount.toStringAsFixed(2) : '');
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    TextField(
                      controller: mealEmployerCtrl,
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      decoration: const InputDecoration(
                        labelText: 'Part employeur Tickets Resto (€)',
                        suffixText: '€',
                        border: OutlineInputBorder(),
                      ),
                    ...
                    TextField(
                      ...
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      decoration: const InputDecoration(
                        labelText: 'Remboursement de Frais (€)',
                        suffixText: '€',
                        border: OutlineInputBorder(),
                      ),
                      style: const TextStyle(color: AppColors.textPrimary, fontSize: 13),
                    ...
                    const SizedBox(height: 12),
                    TextField(
                      controller: rttBuybackCtrl,
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      decoration: const InputDecoration(
                        labelText: 'Rachat RTT / Heures Sup (€)',
                        suffixText: '€',
                        border: OutlineInputBorder(),
                      ),
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    final newMealEmp = double.tryParse(mealEmployerCtrl.text) ?? record.mealTicketsEmployer;
                    final newExpenseReimb = double.tryParse(expenseReimbursementCtrl.text) ?? record.expenseReimbursement;
                    final newRttBuyback = double.tryParse(rttBuybackCtrl.text) ?? record.rttBuybackAmount;
                    ...
                    final updated = record.copyWith(
                      ...
                      mealTicketsEmployer: newMealEmp,
                      expenseReimbursement: newExpenseReimb,
                      rttBuybackAmount: newRttBuyback,
                    );
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () {
                            ...
                            if (currentScale > 0.8) {
                              final newScale = (currentScale - 0.25).clamp(0.8, 3.5);
                        ...
                        Text(
                          '${(_transformationController.value.getMaxScaleOnAxis() * 100).round()}%',
